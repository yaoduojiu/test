...
const user = {
  ...
  actions: {
    ...
    //获取用户绑定的仓库
    Warehouse({ commit }) {
      Promise.all([getAction('/warehouse/warehouse/warehouseByUserId'), getAction('/warehouse/warehouse/listByStatus')]).then(([selected, list]) => {
        if(selected.success){
          commit('SET_WAREHOUSE', selected.result && selected.result.id);
        }
        if(list.success){
          commit('SET_WAREHOUSE_LIST', list.result);
        }
      })
