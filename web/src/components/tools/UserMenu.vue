<template>
  <div class="user-wrapper" :class="theme">
    ...
    <a-select :value="cwarehouse" @change="handleChange" placeholder="请选择仓库" style="min-width: 120px;">
      <a-select-option v-for="item in warehouseList" :value="item.id" :key = "item.id">
        {{ item.name }}
      </a-select-option>
    </a-select>
    <span class="action">
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.cwarehouse = this.warehouse;
      //如果是单点登录模式
    ...
    methods: {
      //切换仓库
      handleChange(value) {
        let params = this.warehouseList.find(item => item.id === value);
        postAction('/warehouse/warehouse/saveWarehouseByUserId', params).then(res => {
          this.cwarehouse = value;
          location.reload();
        })
