...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: "/order/premixOrder/print/list"
      },
  ...
  methods: {
    rePrintClick(record) {
      ...
      this.$confirm({
        ...
        onOk() {
          _this.loading = true
          getAction(`/order/premixOrder/reprint?id=${record.id}&printerName=${_this.printName}&templatePath=${_this.printTemplate}`).then(res => {
            if(res.success) {
              _this.loading = false
